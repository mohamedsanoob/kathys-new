...
export interface CategoryClientData extends CategoryInitialData {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-[100vh]">
        <div className="h-10 w-10 rounded-full border-4 border-gray-200 border-t-green-700 animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col max-w-[1290px] mx-auto md:mt-[1rem] p-1">
      ...
      <div className="mb-4 px-2 md:px-0">
        <CategoryCouponPromo categoryId={currentCategory.id} variant="banner" />
      </div>
          ...
          <div className="relative">
            <div className="overflow-x-auto pb-4 -mx-2 px-2 md:-mx-4 md:px-4 touch-pan-x">
              <div className="inline-flex gap-2 w-max min-w-full md:gap-3">
                {subCategoriesDetails.map((subCategory: any) => (
                  <Link
                    key={subCategory.id}
      ...
      <div className="flex flex-col md:flex-row">
        <FilterSection categoryName={currentCategory?.id} initialFacets={facets} />
        <ProductsSection />
      </div>
    </div>
  );
